...
- **`World.ts`** — simulation step: random roid spawning (in native space), movement + boundary clamping/culling (`boundNTick`; unbound objects cull only when fully off screen), collision *detection* (broad phase + `intersectsWith`, each pair processed once via `Obj.id` ordering, then handed to `resolveCollision`), and a death sweep (`dead` flag or `health <= 0 && !immortal`) — never remove entities by index.
...
- **`Object.ts`** (`Obj`) — base class for all entities: `mass`, `health`, a `Geo`, a `Path2D`, and a unique `id`. Implements `Collidable`: exposes a world-space `center` and an `onHit(ctx)` hook that applies damage; removal is the World's job. Highlighting is a `highlightUntil` timestamp (no timers). Subclasses: `Craft` (player ship: thrusters, weapons switched edge-triggered with Q/E, fires `Projectile`s that inherit craft velocity), `Roid` (asteroids, vertices generated per-instance), `Projectile` (optionally ricochets in `onHit` — `bounce` option in `config.projectile` or per-instance).
- **`Collision.ts`** — the `Collidable` interface (`center`, `health`, `highlight`, `onHit`) and `resolveCollision(a, b)`: given a pair World has confirmed collide, it computes the center-to-center normal once and hands it to each party's `onHit` (negated for the far side). It holds no damage or ricochet policy — that lives in each entity's `onHit`. Collision *resolution*; World owns detection.
